...
func NewLedgerDisk(dbPath string, readOnly, prune bool, plotStore PlotStorage, repGraph *Graph) (*LedgerDisk, error) {
	opts := opt.Options{ReadOnly: readOnly}
	db, err := leveldb.OpenFile(dbPath, &opts)
	if err != nil {
		return nil, err
	}
	return &LedgerDisk{db: db, plotStore: plotStore, repGraph: *&repGraph, prune: prune}, nil
}
...
func (l LedgerDisk) ConnectPlot(id PlotID, plot *Plot) ([]RepresentationID, error) {
	...
	for i, tx := range plot.Representations {
		...
		if txToApply != nil {
			...
			if l.repGraph.IsParentDescendant(pubKeyToString(txToApply.For), pubKeyToString(txToApply.By)){
				txID, _ := txToApply.ID()
				return nil, fmt.Errorf("Sender is a descendant of recipient in representation %s", txID)
			}
		}

		// associate this representation with both parties
		if !tx.IsPlotroot() {
			key, err = computePubKeyRepresentationIndexKey(tx.By, &plot.Header.Height, &i)
			if err != nil {
				return nil, err
			}
			batch.Put(key, []byte{0x1})
		}
		key, err = computePubKeyRepresentationIndexKey(tx.For, &plot.Header.Height, &i)
		if err != nil {
			return nil, err
		}
		batch.Put(key, []byte{0x1})
