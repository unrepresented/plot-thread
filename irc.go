...
// port is our local plotthread port. If it's set to 0 we won't be used for inbound connections.
func (i *IRC) Connect(genesisID PlotID, port int, addrChan chan<- string) error {
...
func generateChannelName(genesisID PlotID, n int) string {
	g := genesisID.String()
	return "#plotthread-" + g[len(g)-8:] + "-" + strconv.Itoa(n)
}
