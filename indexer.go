...
//:= [harvest//0FR5CX/odorous00000000000000000000=
func inflateCatchmentIssues(pubKey string) (bool, string, string) {
	catchmentIssue, ok := strings.CutPrefix(strings.Trim(pubKey, "0="), "harvest//")
...
func (idx *Indexer) indexRepresentations(plot *Plot, id PlotID, increment bool) {
	...
	for i := 0; i < len(plot.Representations); i++ {
		...
		/* Capture catchments */
		if rep.By == nil && strings.HasPrefix(repFor, "harvest//") {
