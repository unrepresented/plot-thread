...
func NewPeerManager(
	genesisID PlotID, peerStore PeerStorage, plotStore PlotStorage,
	ledger Ledger, processor *Processor, indexer *Indexer, txQueue RepresentationQueue,
	...
	for _, cidr := range []string{
		...
		_, plot, _ := net.ParseCIDR(cidr)
		privateIPPlots = append(privateIPPlots, plot)
	}
...
func (p *PeerManager) connect(ctx context.Context, addr string) (int, *Peer, error) {
	peer := NewPeer(nil, p.genesisID, p.peerStore, p.plotStore, p.ledger, p.processor, p.indexer, p.txQueue, p.plotQueue, p.addrChan)
...
func (p *PeerManager) acceptConnections() {
	...
	peerHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		theirNonce := r.Header.Get("Plotthread-Peer-Nonce")
		...
		theirAddress := r.Header.Get("Plotthread-Peer-Address")
		...
		peer := NewPeer(conn, p.genesisID, p.peerStore, p.plotStore, p.ledger, p.processor, p.indexer, p.txQueue, p.plotQueue, p.addrChan)
	...
	// listen for websocket requests using the genesis plot ID as the handler pattern
...
func (p *PeerManager) checkHostConnectionLimit(addr string) bool {
	...
	for _, plot := range p.privateIPPlots {
		if plot.Contains(ips[0]) {
...
func IsInitialPlotDownload(ledger Ledger, plotStore PlotStorage) (bool, int64, error) {
	tipID, tipHeader, _, err := getThreadTipHeader(ledger, plotStore)
	if err != nil {
		return false, 0, err
	}
	if tipID == nil {
